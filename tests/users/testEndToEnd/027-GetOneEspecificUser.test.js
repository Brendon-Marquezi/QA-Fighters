...
beforeEach(() => {
  // Configuração do Request Manager antes de cada teste
...
test('Create a user if not exists and then get specific user from Jira', async () => {
  logger.info('Starting test: Create a user if not exists and then get specific user from Jira');
  ...
  const displayName = email.split('@')[0]; // Pega a parte do email antes do '@'
  ...
  if (allUsersResponse.status === 200) {
    const existingUser = allUsersResponse.data.find(user => user.emailAddress === email);
    ...
    logger.error(`Failed to fetch all users. Status: ${allUsersResponse.status}`);
  }
  ...
  logger.debug('Create user response data:', createUserResponse.data);
  ...
  logger.debug('Get user response data:', getResponse.data);

  expect(getResponse.status).toBe(200);

  const validationGet = validateSchema(getResponse.data, getUserResponseSchema);
