...
// Check if the user was actually removed
  ...
  if (verifyGroupResponse.status === 200) {
    ...
    if (!userInGroup) {
      logger.info(`User ${userIdToAdd} successfully removed from group ${createdGroupId}.`);
    } else {
      logger.error(`User ${userIdToAdd} was not removed from group ${createdGroupId}.`);
    }
  } else {
    logger.error(`Group verification failed. Status: ${verifyGroupResponse.status}`);
  }
});
